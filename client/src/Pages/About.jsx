...
function About() {
  const { getTotlaCourses, getTotlaUsers, getTotlaTeachers, getTotlaAdmins } = useContext(ShopContext);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const teacher = await getTotlaTeachers();
        const admin = await getTotlaAdmins();
        setTotalStudents(users-teacher-admin);
      } catch (error) {
        console.error("Error fetching total courses:", error);
      }
      try {
        const teacher = await getTotlaTeachers();
        setTotalTeachers(teacher);
      } catch (error) {
        console.error("Error fetching total courses:", error);
      }
    }
    ...
    <div>
      <div className='big-contain'>
        ...
        <div className='container-about'>
          ...
          <div className='stats'>
            <div className='counter'><div className='avatar'><img src={student} /><h2>Total Students</h2></div><p>{totalStudents}</p></div>
            <div className='counter'><div className='avatar'><img src={prof} /><h2>Total Teachers</h2></div><p>{totalTeachers}</p></div>
            <div className='counter'><div className='avatar'><img src={book} /><h2>Total Courses</h2></div><p>{totalCourses}</p></div>
          </div>
